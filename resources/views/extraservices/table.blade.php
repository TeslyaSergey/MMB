...
<div class="box box-primary">
	...
	<div class="box-body">
	 ...
	 <?php echo Form::open(array('url'=>'extraservices/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
        ...
        <tbody>
			...
			<tr id="form-0" >
			  ...
			  @endif
           		...
           		?>
                <tr class="editable" id="form-{{ $row->extraserviceID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->extraserviceID ;?>" />  </td>
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->extraserviceID }}" data-url="{{ url('extraservices/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>
					@endif
				 <td data-values="action" style="width: 6%;" data-key="<?php echo $row->extraserviceID ;?>"  >
					{!! \App\Library\AjaxHelpers::buttonAction_expense('extraservices',$access,$id ,$setting) !!}
					{!! \App\Library\AjaxHelpers::buttonActionInline($row->extraserviceID,'extraserviceID') !!}
					 ...
					 <?php foreach ($tableGrid as $field) :
					 	if($field['view'] =='1') :
							$value = \App\Library\SiteHelpers::formatRows($row->{$field['field']}, $field , $row);
							 ...
							 <?php if ($field['field'] == 'extraserviceID') {
								 $id = $row->{$field['field']};
										continue;
										} ?>
							 
						 	@if(\App\Library\SiteHelpers::filterColumn($limited ))
							 		@if($field['field'] == 'currencyID')
										<td align="<?php echo $field['align'];?>" data-values="{{ $row->{$field['field']} }}" data-field="{{ $field['field'] }}" data-format="{{ htmlentities($value) }}">
										{!!   \App\Http\Controllers\ExtraexpensesController::currency_symbol($value) !!}
										</td>
									@else
										<td align="<?php echo $field['align'];?>" data-values="{{ $row->{$field['field']} }}" data-field="{{ $field['field'] }}" data-format="{{ htmlentities($value) }}">
											{!! $value !!}
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->extraserviceID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
