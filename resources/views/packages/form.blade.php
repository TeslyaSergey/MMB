                  ...
                  <fieldset id="part_{{$index}}" style="display:none;"><legend>{{ Lang::get('core.part') }}_{{$index+1}}</legend>
                    ...
                    <div class="form-group" >
                      ...
                      <div class="col-md-5" style="padding: 0;">
                        <div class="row">
                          <div class="col-md-5">
                          <div class="input-group m-b" id="dpd1">
                            {!! Form::text('part_start[]', '', array('class'=>'form-control date', 'id'=>'part_start_'.$index, 'autocomplete'=>'off')) !!}
                            <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                          </div>
                          </div>
                          <label for="End Date" class=" control-label col-md-2 text-left"> {{ Lang::get('core.end') }} <span class="asterix"> * </span></label>
                          <div class="col-md-5">
                            <div class="input-group m-b" id="dpd2">
                              {!! Form::text('part_end[]', '', array('class'=>'form-control date',  'id'=>'part_end_'.$index, 'autocomplete'=>'off')) !!}
                              <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div class="col-md-5" style="padding: 0;">
                        <div class="row">
                          ...
                          <label for="Hotel " class=" control-label col-md-2 text-left"> {{ Lang::get('core.hotel') }}  </label>
                          <div class="col-md-5">
...
<div class="modal fade in" id="tickets_modal"  role="dialog" style=" padding-right: 16px;">
                      ...
                      ?>
                       ...
                       <td data-values="{{ $row_f->airlinesID }}" data-field="airlinesID">
                         @foreach($airlines as $a)
                             <?php
                             $airlineIds = json_decode($row_f->airlinesID);
   ...
   @foreach($roomTypes as $roomType)
      room_types[{{$i}}] = [];
      room_types[{{$i}}]['id'] = '{{$roomType->roomtypeID}}';
      room_types[{{$i}}]['type'] = '{{$roomType->room_type}}';
      @php
        $i++;
      @endphp
   @endforeach
   rest_room_types = room_types.slice(0);
      ...
      if(counter>0){
        ...
        @foreach($roomfeatures as $roomfeature)
          ...
          for(room_type of room_types){
            if(room_type['id'] != '{{$roomfeature->roomtypeID}}')
              $("#room_typeID_{{$i}}").append("<option value='"+room_type['id']+"'>"+room_type['type']+"</option>");
            else {
              $("#room_typeID_{{$i}}").append("<option value='"+room_type['id']+"' selected>"+room_type['type']+"</option>");
            }
          }
        ...
        for(room_type of room_types){
          $("#room_typeID_0").append("<option value='"+room_type['id']+"'>"+room_type['type']+"</option>");
        }
        $("#room_feature_0").show();
    ...
    @foreach($parts as $part)
      ...
      $("#part_end_"+index).val('{{$part->part_end}}');
  ...
  $("#addRoomFeature").click(function(){
    ...
    if(next < 5){
      for(room_type of rest_room_types){
        $("#room_typeID_"+next).append("<option value='"+room_type['id']+"'>"+room_type['type']+"</option>");
      }
    ...
    if(last > 0){
      ...
      if($("#room_feature_"+last).val() > 0){
        ...
        for(var i=0;i<rest_room_types.length; i++){
          if(rest_room_types[i]['id'] > $("#room_typeID_"+last).val()){
            var room_type = [];
            room_type['id'] = $("#room_typeID_"+last).val();
            room_type['type'] = $("#room_typeID_"+last).find("option:selected").text();
      ...
      for(var i = 0; i < $("#counter").val(); i++){
        ...
        if($("#room_typeID_"+i).val() > 0){
          ...
          for(var j=0;j<temp_rest_room_types.length; j++){
            if(temp_rest_room_types[j]['id'] > $("#room_typeID_"+i).val()){
              var room_type = [];
              room_type['id'] = $("#room_typeID_"+i).val();
              room_type['type'] = $("#room_typeID_"+i).find("option:selected").text();
        ...
        for(var j = 0; j<temp_rest_room_types.length; j++ ){
          if(temp_rest_room_types[j]['id'] != pre_seleted_val){
            $("#room_typeID_"+i).append("<option value='"+temp_rest_room_types[j]['id']+"'>"+temp_rest_room_types[j]['type']+"</option>");
          }
          else{
            $("#room_typeID_"+i).append("<option value='"+temp_rest_room_types[j]['id']+"' selected>"+temp_rest_room_types[j]['type']+"</option>");
          }
        }
      }
    }
  ...
  function change_room_feature(element){
    ...
    for(var i=0; i<rest_room_types.length; i++){
      if(selected_types.includes(rest_room_types[i]['id'])){
  ...
  $("input[name='total_capacity'],input[name='cost'], input[name='cost_single[]'], input[name='cost_double[]'], input[name='cost_triple[]'], input[name='cost_child[]'], input[name='parts'], input[name='seat[]'], input[name='cost[]']").TouchSpin();
</script>
